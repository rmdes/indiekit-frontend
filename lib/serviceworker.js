...
/**
 * Remove any cached auth/session/preview pages from the pages cache.
    ...
    await Promise.all(
      keys
        .filter((request) => {
          ...
          // Purge auth/session (stale auth state) and preview (preview pages
          // must never be cached — revision polling needs fresh fetches and
          // preview tokens rotate on publish, so cached entries are dead weight).
          return (
            url.origin === location.origin &&
            /^\/(auth|session|preview)(?:\/|$)/.test(url.pathname)
          );
        })
...
addEventListener("fetch", (event) => {
  ...
  // Never cache auth/session/preview pages — always go to network.
  ...
  // Preview pages (/preview/<token>/) must never be cached: the composition
  // editor polls data-preview-revision and needs every fetch to hit the
  // network for a fresh build, and preview tokens rotate on publish so
  // cached entries would be dead weight.
  ...
  event.respondWith(
    (async () => {
      try {
        ...
        // Fetch from the network and refresh the image cache; returns
        // undefined when the network is unavailable
        const revalidate = async () => {
          try {
            ...
            console.error("Background fetch failed:", error, request.url);
          }
        };
        ...
        const responseFromFetch = await revalidate();
